...
show_usage() {
    ...
    echo "  --motion-audio PATH     Set the motion audio directory for real-robot playback (optional)"
    ...
    echo "  $0 --motion-data reference/custom_motion/ --motion-audio reference/custom_audio/ real"
}
...
MOTION_AUDIO="$MOTION_AUDIO_DEFAULT"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --motion-audio)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --motion-audio requires a path argument${NC}" >&2
                exit 1
            fi
            MOTION_AUDIO="$2"
            shift 2
...
echo -e "  Motion Audio:       ${GREEN}$MOTION_AUDIO${NC}"
fi
...
if [[ -n "$MOTION_AUDIO" ]]; then
echo -e "${BLUE}    --motion-audio-dir $MOTION_AUDIO${NC}"
else
echo -e "${BLUE}    # motion audio disabled${NC}"
fi
if [[ -n "$EXTRA_ARGS" ]]; then
