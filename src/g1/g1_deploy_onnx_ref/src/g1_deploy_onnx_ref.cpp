...
std::string ResolveMotionAudioDirectory(const std::string& motion_data_path,
                                        const std::string& requested_audio_dir) {
  namespace fs = std::filesystem;

  auto normalize_path = [](const fs::path& path) {
    std::error_code ec;
    const fs::path weakly_canonical = fs::weakly_canonical(path, ec);
    return ec ? path.lexically_normal() : weakly_canonical;
  };
  ...
  if (!requested_audio_dir.empty()) {
    const fs::path explicit_dir = normalize_path(requested_audio_dir);
    if (!is_directory(explicit_dir)) {
      throw std::runtime_error("Motion audio directory does not exist: " +
                               explicit_dir.string());
    }
    return explicit_dir.string();
  }

  const fs::path motion_data_dir = normalize_path(motion_data_path);
  const fs::path motion_parent_dir = motion_data_dir.parent_path();
  const std::vector<fs::path> candidates = {
      motion_data_dir / "audio",
      motion_data_dir / "audio_en",
      motion_data_dir / "audio_aligned",
      motion_data_dir / "audio_aligned_en",
      motion_data_dir / "audio_16k",
      motion_data_dir / "audio_16k_en",
      motion_data_dir / "audio_aligned_16k",
      motion_data_dir / "audio_aligned_16k_en",
      motion_parent_dir / "audio",
      motion_parent_dir / "audio_en",
      motion_parent_dir / "audio_aligned",
      motion_parent_dir / "audio_aligned_en",
      motion_parent_dir / "audio_16k",
      motion_parent_dir / "audio_16k_en",
      motion_parent_dir / "audio_aligned_16k",
      motion_parent_dir / "audio_aligned_16k_en",
  };
...
class G1Deploy {
  private:
    ...
    bool use_sdk_startup_stand_ = false;
    bool sdk_startup_handoff_pending_ = false;
    std::array<double, G1_NUM_MOTOR> sdk_startup_hold_pose_{};
    ...
    // Stand-reference transition for reference-motion observations only.
    // This keeps low-level motor control unchanged and only smooths the
    // motion observations seen by the policy/encoder.
    static constexpr double kStandReferenceTransitionDurationSec = 1.00;
    static constexpr double kStandReferenceRootHeightM = 0.80;
    static constexpr std::array<double, 4> kStandReferenceRootQuatWxyz{
        {0.70710678, 0.0, 0.0, -0.70710678}};
    enum class StandReferencePhase { NONE, TO_MOTION_START, TO_DEFAULT_STAND };
    struct StandReferenceTransitionState {
      StandReferencePhase phase = StandReferencePhase::NONE;
      std::shared_ptr<const MotionSequence> motion = nullptr;
      int frozen_frame = 0;
      std::chrono::steady_clock::time_point started_at{};
      double duration_sec = kStandReferenceTransitionDurationSec;
    };
    StandReferenceTransitionState stand_reference_transition_;
    ...
    static double SmoothStep01(double alpha) {
      const double clamped = std::clamp(alpha, 0.0, 1.0);
      return clamped * clamped * (3.0 - 2.0 * clamped);
    }

    void ResetStandReferenceTransition() {
      stand_reference_transition_.phase = StandReferencePhase::NONE;
      stand_reference_transition_.motion.reset();
      stand_reference_transition_.frozen_frame = 0;
      stand_reference_transition_.started_at = std::chrono::steady_clock::time_point{};
      stand_reference_transition_.duration_sec = kStandReferenceTransitionDurationSec;
    }

    bool HasActiveStandReferenceTransition() const {
      return stand_reference_transition_.phase != StandReferencePhase::NONE &&
             stand_reference_transition_.motion != nullptr;
    }

    bool HasStandReferenceTransitionForCurrentMotion() const {
      return HasActiveStandReferenceTransition() &&
             stand_reference_transition_.motion == current_motion_;
    }

    double GetStandReferenceTransitionAlpha() const {
      if (!HasActiveStandReferenceTransition()) {
        return 1.0;
      }
      const double duration = std::max(1e-3, stand_reference_transition_.duration_sec);
      const double elapsed = std::chrono::duration<double>(
          std::chrono::steady_clock::now() - stand_reference_transition_.started_at)
          .count();
      return SmoothStep01(elapsed / duration);
    }

    bool IsStandReferenceTransitionComplete() const {
      return HasActiveStandReferenceTransition() &&
             GetStandReferenceTransitionAlpha() >= 1.0 - 1e-6;
    }

    void StartStandReferenceTransitionToMotionStart(
        const std::shared_ptr<const MotionSequence>& motion) {
      if (!IsReferencePlaybackMotion(motion) || motion->timesteps <= 0) {
        ResetStandReferenceTransition();
        return;
      }
      stand_reference_transition_.phase = StandReferencePhase::TO_MOTION_START;
      stand_reference_transition_.motion = motion;
      stand_reference_transition_.frozen_frame = 0;
      stand_reference_transition_.started_at = std::chrono::steady_clock::now();
      stand_reference_transition_.duration_sec = kStandReferenceTransitionDurationSec;
    }

    void StartStandReferenceTransitionToDefault(
        const std::shared_ptr<const MotionSequence>& motion, int frame) {
      if (!IsReferencePlaybackMotion(motion) || motion->timesteps <= 0) {
        ResetStandReferenceTransition();
        return;
      }
      stand_reference_transition_.phase = StandReferencePhase::TO_DEFAULT_STAND;
      stand_reference_transition_.motion = motion;
      stand_reference_transition_.frozen_frame = ClampFrameIndex(motion, frame);
      stand_reference_transition_.started_at = std::chrono::steady_clock::now();
      stand_reference_transition_.duration_sec = kStandReferenceTransitionDurationSec;
    }

    bool ShouldHoldStandReference() const {
      return IsReferencePlaybackMotion(current_motion_) &&
             !operator_state.play &&
             !HasActiveStandReferenceTransition();
    }

    double ApplyStandReferenceToScalar(double motion_value, double stand_value) const {
      if (HasStandReferenceTransitionForCurrentMotion()) {
        const double alpha = GetStandReferenceTransitionAlpha();
        if (stand_reference_transition_.phase == StandReferencePhase::TO_MOTION_START) {
          return (1.0 - alpha) * stand_value + alpha * motion_value;
        }
        if (stand_reference_transition_.phase == StandReferencePhase::TO_DEFAULT_STAND) {
          return (1.0 - alpha) * motion_value + alpha * stand_value;
        }
      }
      if (ShouldHoldStandReference()) {
        return stand_value;
      }
      return motion_value;
    }
    ...
    template <size_t N>
    std::array<double, N> ApplyStandReferenceToArray(
        const std::array<double, N>& motion_value,
        const std::array<double, N>& stand_value) const {
      std::array<double, N> out = motion_value;
      if (HasStandReferenceTransitionForCurrentMotion()) {
        const double alpha = GetStandReferenceTransitionAlpha();
        if (stand_reference_transition_.phase == StandReferencePhase::TO_MOTION_START) {
          for (size_t i = 0; i < N; ++i) {
            out[i] = (1.0 - alpha) * stand_value[i] + alpha * motion_value[i];
          }
          return out;
        }
        if (stand_reference_transition_.phase == StandReferencePhase::TO_DEFAULT_STAND) {
          for (size_t i = 0; i < N; ++i) {
            out[i] = (1.0 - alpha) * motion_value[i] + alpha * stand_value[i];
          }
          return out;
        }
      }
    ...
    std::string DescribeMotionMode(const std::string& form,
    ...
    bool WaitForMotionModeName(const std::string& expected_name,
                               bool expect_active,
    ...
    bool TrySelectMotionMode(const std::vector<std::string>& candidates,
                             std::string* selected_name = nullptr) {
      for (const auto& candidate : candidates) {
        const int32_t select_ret = msc_->SelectMode(candidate);
        if (select_ret == 0) {
          if (selected_name) {
        ...
        std::cout << "SelectMode(\"" << candidate << "\") failed. ret="
                  << select_ret << std::endl;
      }
      return false;
    }

    bool RunSdkStandSequence() {
      if (!loco_client_) {
        return false;
      }

      std::cout << "Requesting Unitree SDK stand-up before low-level takeover."
                << std::endl;
      const int32_t stop_ret = loco_client_->StopMove();
      if (stop_ret != 0) {
        std::cout << "SDK StopMove returned ret=" << stop_ret
                  << ", continuing with stand-up request." << std::endl;
      }

      const int32_t stand_ret = loco_client_->StandUp();
      if (stand_ret != 0) {
        std::cout << "SDK StandUp failed. ret=" << stand_ret << std::endl;
        return false;
      }
      std::this_thread::sleep_for(std::chrono::seconds(4));

      const int32_t balance_ret = loco_client_->BalanceStand();
      if (balance_ret != 0) {
        std::cout << "SDK BalanceStand failed. ret=" << balance_ret << std::endl;
        return false;
      }

      const int32_t stop_after_balance_ret = loco_client_->StopMove();
      if (stop_after_balance_ret != 0) {
        std::cout << "SDK StopMove after BalanceStand returned ret="
                  << stop_after_balance_ret << std::endl;
      }

      std::this_thread::sleep_for(std::chrono::milliseconds(1500));
      sdk_startup_handoff_pending_ = true;
      std::cout << "SDK stand is ready. Releasing motion service for low-level control."
                << std::endl;
      return true;
    }

    bool EnsureSdkStandModeReady() {
      std::string form, name;
      const int32_t ret = msc_->CheckMode(form, name);
      if (ret != 0) {
        std::cout << "Failed to query motion mode before SDK stand. ret="
                  << ret << ". Will try fallback selection paths." << std::endl;
      } else {
        if (name == "normal") {
          return true;
        }
        if (!name.empty()) {
          std::cout << "Current motion mode before SDK stand: "
                    << DescribeMotionMode(form, name) << std::endl;
          // G1 can already be in a valid high-level mode like ai_sport.
          return true;
        }
      }

      std::cout << "Trying to activate a supported high-level motion mode for SDK stand."
                << std::endl;
      std::string selected_name;
      if (!TrySelectMotionMode({"normal", "sport_mode", "ai", "ai_sport",
                                "advanced", "advanced_sport"},
                               &selected_name)) {
        return false;
      }

      if (!WaitForMotionModeName(selected_name, true, std::chrono::seconds(8))) {
        // Some robots report aliases in CheckMode differently from SelectMode input.
        // If selection succeeded, allow the next SDK call to be the actual probe.
        std::cout << "Selected mode \"" << selected_name
                  << "\" but CheckMode confirmation timed out; continuing to probe with SDK stand."
                  << std::endl;
      }
      return true;
    }

    bool PrepareRobotWithSdkStand() {
      if (!msc_ || !loco_client_) {
        return false;
      }
      if (RunSdkStandSequence()) {
        return true;
      }

      if (!EnsureSdkStandModeReady()) {
        std::cout << "Unable to prepare a supported SDK motion mode for startup stand."
                  << std::endl;
        return false;
      }
      return RunSdkStandSequence();
    }
    ...
    bool GatherMotionRootZPositionMultiFrame(std::vector<double>& target_buffer, size_t offset, int num_frames = 5, int step_size = 5) {
      ...
      for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
        ...
        motion_root_z_pos =
            ApplyStandReferenceToScalar(motion_root_z_pos, kStandReferenceRootHeightM);
    ...
    bool GatherMotionAnchorOrientationMutiFrame(std::vector<double>& target_buffer, size_t offset, int num_frames = 5, int step_size = 5) {
      ...
      for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
        ...
        auto stand_ref_root_rot = quat_mul_d(apply_delta_heading, kStandReferenceRootQuatWxyz);
        auto stand_base_to_ref_quat =
            quat_mul_d(quat_conjugate_d(base_quat), stand_ref_root_rot);
        auto stand_rotation_matrix = quat_to_rotation_matrix_d(stand_base_to_ref_quat);
        std::array<double, 6> stand_anchor_ori_b = {
            stand_rotation_matrix[0][0], stand_rotation_matrix[0][1],
            stand_rotation_matrix[1][0], stand_rotation_matrix[1][1],
            stand_rotation_matrix[2][0], stand_rotation_matrix[2][1]};
        motion_anchor_ori_b =
            ApplyStandReferenceToArray(motion_anchor_ori_b, stand_anchor_ori_b);
    ...
    bool GatherMotionJointPositionsMultiFrame(std::vector<double>& target_buffer, size_t offset, int num_frames = 5, int step_size = 5, std::vector<int> joint_indexes = {}) {
      ...
      for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
        ...
        const std::array<double, 29> stand_joint_pos{};
        blended_joint_pos = ApplyStandReferenceToArray(blended_joint_pos, stand_joint_pos);
  ...
  public:
    ...
    G1Deploy(
      ...
      const auto resolved_motion_audio_dir =
          ResolveMotionAudioDirectory(motion_data_path, motion_audio_dir);
      if (!resolved_motion_audio_dir.empty()) {
        std::cout << "Motion-synced audio enabled from: "
                  << resolved_motion_audio_dir << std::endl;
      } else {
        std::cout << "Motion-synced audio disabled (no audio directory found)."
                  << std::endl;
      }
      audio_thread_ =
          std::make_unique<AudioThread>(resolved_motion_audio_dir);
      ...
      use_sdk_startup_stand_ = !disable_crc_check_;
      if (use_sdk_startup_stand_) {
        std::cout << "Running Unitree SDK startup stand before enabling low-level control."
                  << std::endl;
        if (!PrepareRobotWithSdkStand()) {
          use_sdk_startup_stand_ = false;
          sdk_startup_handoff_pending_ = false;
          std::cout << "SDK startup stand is unavailable on this robot/session."
                    << " Falling back to legacy low-level init ramp."
                    << std::endl;
        }
      } else {
        std::cout << "Skipping Unitree SDK startup stand because CRC checking is disabled."
                  << std::endl;
      }
      std::string form, name;
    ...
    bool InitControl() {
      ...
      MotorCommand motor_command_tmp;
      if (sdk_startup_handoff_pending_) {
        for (int i = 0; i < G1_NUM_MOTOR; ++i) {
          sdk_startup_hold_pose_.at(i) = ls->motor_state()[i].q();
          motor_command_tmp.tau_ff.at(i) = 0.0;
          motor_command_tmp.q_target.at(i) =
              static_cast<float>(sdk_startup_hold_pose_.at(i));
          motor_command_tmp.dq_target.at(i) = 0.0;
        ...
        std::cout << "SDK stand handoff complete. Holding official stand pose."
                  << std::endl;
        return true;
      }
    ...
    bool GatherInputInterfaceData() {
      ...
      bool motion_audio_enabled = false;
      {
        std::lock_guard<std::mutex> lock(current_motion_mutex_);
        if (current_motion_) {
          current_motion_name = current_motion_->name;
      ...
      audio_thread_->SetCommand(
        AudioCommand{
          ...
          .motion_audio_enabled = motion_audio_enabled,
          .motion_playing = operator_state.play,
    ...
    bool CurrentFrameAdvancement() {
      ...
      if (HasActiveStandReferenceTransition()) {
        if (!HasStandReferenceTransitionForCurrentMotion()) {
          ResetStandReferenceTransition();
        } else if (stand_reference_transition_.phase == StandReferencePhase::TO_MOTION_START) {
          current_frame_ = 0;
          if (IsStandReferenceTransitionComplete()) {
            ResetStandReferenceTransition();
            operator_state.play = true;
            std::cout << "Stand-to-motion observation transition completed." << std::endl;
          }
          return true;
        } else if (stand_reference_transition_.phase == StandReferencePhase::TO_DEFAULT_STAND) {
          current_frame_ = stand_reference_transition_.frozen_frame;
          if (IsStandReferenceTransitionComplete()) {
            current_frame_ = 0;
            ResetStandReferenceTransition();
            reinitialize_heading_ = true;
            std::cout << "Motion-to-stand observation transition completed." << std::endl;
          }
          return true;
        }
      }
      ...
      if (!use_planner_motion && current_motion_ && current_motion_->timesteps > 0 && operator_state.play) {
        ...
        if (current_motion_->name != "streamed") {
          if (current_frame_ >= current_motion_->timesteps) {
            const int last_frame = std::max(0, current_motion_->timesteps - 1);
            StartStandReferenceTransitionToDefault(current_motion_, last_frame);
            operator_state.play = false;
            std::cout << "Motion " << current_motion_->name << " completed." << std::endl;
            current_frame_ = last_frame;
            std::cout << "Transitioning observation back to stand reference." << std::endl;
          }
        } else {
    ...
    void Input() {
      ...
      bool play_after_input = false;
      {
        ...
        if (HasActiveStandReferenceTransition() &&
            stand_reference_transition_.motion != motion_after_input) {
          ResetStandReferenceTransition();
        }

        const bool play_rising_edge = !play_before_input && play_after_input;
        if (play_rising_edge && IsReferencePlaybackMotion(motion_after_input) &&
            motion_after_input->timesteps > 0) {
          operator_state.play = false;
          play_after_input = false;
          current_frame_ = 0;
          frame_after_input = 0;
          ResetMotionSwitchBlend();
          StartStandReferenceTransitionToMotionStart(motion_after_input);
          std::cout << "Transitioning observation from stand reference to motion start." << std::endl;
        }
      }
      if (motion_before_input && motion_after_input) {
        ...
        if (planner_involved) {
          ResetMotionSwitchBlend();
        } else if (motion_before_input != motion_after_input && play_after_input) {
          ...
          const bool rewound = play_after_input && (frame_after_input + 2 < frame_before_input);
          if (rewound) {
            StartMotionSwitchBlend(motion_before_input, frame_before_input, motion_after_input);
          }
        }
...
int main(int argc, char const* argv[]) {
  ...
  if (argc < 4) {
    ...
    std::cout << "  --motion-audio-dir <path>: play matching WAV files on the robot speaker during reference motions" << std::endl;
  ...
  for (int i = 4; i < argc; i++) {
    ...
    } else if (std::string(argv[i]) == "--motion-audio-dir" ||
               std::string(argv[i]) == "--motion-audio") {
      if (i + 1 < argc) {
        motionAudioDir = argv[i + 1];
        std::cout << "[INFO] Using motion audio directory: " << motionAudioDir
                  << std::endl;
        i++;
      } else {
        std::cerr << "Error: --motion-audio-dir requires a path argument"
                  << std::endl;
        exit(1);
      }
